...
if key_of_interest in data:
    ...
    if len(variable_data.shape) == 3:
        ...
        original_tiff_file = f'/home/arawa/Shabaz_simulation/figure_7_ak/segmentedSinusoids_Levy_walks_liver/IMG_OPENED_3D_yx{original_shape[1]}_z{original_shape[0]}.tiff'
        tiff.imwrite(original_tiff_file, reordered_data.astype(np.float32))  # Save as 32-bit TIFF
        npz_file = f'/home/arawa/Shabaz_simulation/figure_7_ak/segmentedSinusoids_Levy_walks_liver/IMG_OPENED_3D_yx{original_shape[1]}_z{original_shape[0]}.npz'
        ...
        # Repeat the data along the z-axis (depth) 13 times to extend the z dimension
        expanded_data = np.tile(reordered_data, (13, 1, 1))  # Repeat along z-axis
        expanded_shape = expanded_data.shape
        print(f"Expanded shape: {expanded_shape}")

        # Save the expanded data
        expanded_tiff_file = f'/home/arawa/Shabaz_simulation/figure_7_ak/segmentedSinusoids_Levy_walks_liver/IMG_OPENED_3D_{expanded_shape[1]}x{expanded_shape[2]}_z{expanded_shape[0]}.tiff'
        tiff.imwrite(expanded_tiff_file, expanded_data.astype(np.float32))  # Save as 32-bit TIFF
        expanded_npz_file = f'/home/arawa/Shabaz_simulation/figure_7_ak/segmentedSinusoids_Levy_walks_liver/IMG_OPENED_3D_{expanded_shape[1]}x{expanded_shape[2]}_z{expanded_shape[0]}.npz'
